...
namespace DataUtil
{
    ...
    /// </summary>
    public sealed class SQLFunctions : DatabaseFunctions
    {
        ...
        /// Sql connection
        /// </summary>
        private SqlConnection m_Connection { get; set; }

        /// <summary>
        /// Sql connection information object
        /// </summary>
        private SQLConnectionInformation m_SQLInfo = null;
        ...
        public SQLFunctions(SQLConnectionInformation sqlConnectionInformation)
        {
            this.m_SQLInfo = sqlConnectionInformation;

            string strConnectionString = this.m_SQLInfo.ConnectionString;
        ...
        public SQLFunctions(SQLConnectionInformation.AuthenticationType authenticationType, string strServerName, string strDatabaseName)
        {
            this.m_SQLInfo = new SQLConnectionInformation(authenticationType, strServerName, strDatabaseName);
        ...
        public SQLFunctions(SQLConnectionInformation.AuthenticationType authenticationType, string strServerName, string strDatabaseName, string strUserId, string strPassword)
        {
            this.m_SQLInfo = new SQLConnectionInformation(authenticationType, strServerName, strDatabaseName, strUserId, strPassword);
        ...
        public override Globals.ResultType TryConnect(ref string strError)
        {
            ...
            if (this.IsConnected == false || (this.m_Connection != null && this.m_Connection.State == ConnectionState.Closed))
            {
                ...
                {
                    strError += "Error: The connection to server '" + this.m_SQLInfo.DatabaseServer + "' failed.";
        ...
        public override DataTable GetDataTable(string strQuery, ref string strError, int intDefaultTimeout = 300)
        {
            try
            {
                ...
                // Create DataAdapter
                SqlDataAdapter dataAdapter = new SqlDataAdapter(strQuery, this.m_Connection);
                dataAdapter.SelectCommand.CommandTimeout = intDefaultTimeout;
        ...
        public override T ExecuteScalar<T>(string strQuery, ref string strError)
        {
            try
            {
                ...
                // Create SqlCommand
                SqlCommand cmd = new SqlCommand(strQuery, this.m_Connection);
        ...
        /// <returns></returns>
        public SqlDataReader ExecuteReader(string strQuery, ref string strError, CommandBehavior behavior = CommandBehavior.Default)
        {
            try
            {
                // Validation
                ...
                // Create Data Reader
                SqlDataReader reader = this.ExecuteReader(cmd, ref strError, behavior);
        ...
        /// <param name="behavior">Provides a description of the results of the query and its effect on the database</param>
        /// <returns></returns>
        public SqlDataReader ExecuteReader(SqlCommand cmd, ref string strError, CommandBehavior behavior = CommandBehavior.Default)
        {
            try
            {
                // Ensure Connected
        ...
        public override int ExecuteNonQuery(string strQuery, ref string strError)
        {
            try
            {
                ...
                // Execute Non Query
                int intRowsAffected = this.ExecuteNonQuery(cmd, ref strError);

                // Validation
                if (strError != "") { return -1; }
        ...
        /// <param name="strError">Error string containing any Error message encountered</param>
        /// <returns></returns>
        public int ExecuteNonQuery(SqlCommand cmd, ref string strError)
        {
            try
            {
                // Ensure Connected
